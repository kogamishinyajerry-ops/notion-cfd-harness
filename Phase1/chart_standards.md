...
- 基准数据（实验/解析解）：空心圆圈 `marker='o', facecolors='none'`
- CFD数据：实心圆点 `marker='o', facecolors='blue'`
- 误差棒：显示在数据点上（error bar 长度 = 2σ 或实验不确定度）

### 1.2.1 多数据集标记样式枚举
当报告对比 3+ 组数据时，强制使用以下标记区分：
| 标记样式 | 数据类型 | matplotlib 参数 |
|---------|---------|--------------|
| 空心圆圈 | 实验/解析解基准 | `marker='o', facecolors='none'` |
| 实心圆点 | CFD 计算结果 | `marker='o', facecolors='C0'` |
| 上三角 | 网格收敛中间解 | `marker='^'` |
| 下三角 | 文献数据对比 | `marker='v'` |
| 方块 | 其他湍流模型对比 | `marker='s'` |
| 菱形 | 网格敏感性分析 | `marker='D'` |
