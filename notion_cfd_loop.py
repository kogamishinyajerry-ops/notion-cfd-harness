...
def sync_task_to_notion(task_id: str, updates: dict) -> bool:
    ...
    try:
        ...
        for key, value in updates.items():
            if key == "执行日志":
                properties["执行日志"] = {"rich_text": [{"text": {"content": str(value)}}]}
            elif key == "phase状态":
                status_map = {
                    "规划中": "待规划", "开发中": "开发中",
                    "审查中": "待审查", "已完成": "已完成"
                }
                properties["phase状态"] = {"status": {"name": status_map.get(value, value)}}
            elif key == "phase编号":
                properties["phase编号"] = {"number": int(value)}
...
def query_pending_tasks(status_filter: str = None) -> list:
    """
    查询待处理任务
    status_filter: "待规划" / "开发中" / "待审查" / etc.
    """
    query = {
        "database_id": SSOT_DB_ID,
        "page_size": 50
    }
    ...
    if status_filter:
        ...
        for t in all_results:
            status = t.get('properties',{}).get('phase状态',{}).get('status',{}).get('name','')
...
def create_notion_page_task(parent_id: str, title: str, content: str, task_type: str) -> str:
    """
    在 Notion 中创建一个任务/指令页面
    Claude Code 读取这个页面就知道要执行什么
    返回 page_id
    """
    # 根据类型选择 icon
    icon_map = {"指令": "🎯", "分析": "🔍", "审查": "✅", "学习": "📚", "任务": "📋"}
        ...
        "children": [
            {
                ...
                "callout": {
                    "rich_text": [{"text": {"content": f"🤖 Claude Code 待执行 {task_type} | 创建时间: {datetime.now().strftime('%Y-%m-%d %H:%M')}"}}],
                    "icon": {"emoji": icon}, "color": "blue"
                }
            },
            {
                "object": "block", "type": "heading_2",
                "heading_2": {"rich_text": [{"text": {"content": "📋 任务描述"}}]}
            },
            {
            ...
            {
                "object": "block", "type": "heading_2",
                "heading_2": {"rich_text": [{"text": {"content": "🎯 Claude Code 执行指令"}}]}
            },
            {
                "object": "block", "type": "code",
                "code": {"rich_text": [{"text": {"content": f"请执行以下任务，完成后调用 sync_task_to_notion() 同步结果到 Notion。"}}], "language": "plain text"}
            }
        ]
    })
    return page.get("id", "")
        ...
        if not pending:
            ...
            for status, tasks in pending.items():
                ...
                for task in tasks[:3]:  # 只显示前3个
                    title = task.get("properties", {}).get("项目名称", {}).get("title", [{}])
...
# 传令协议：Claude Code ↔ Notion AI 通过执行日志字段进行结构化交接
...
def get_page_execution_log(page_id: str) -> Tuple[str, Dict]:
    ...
    for prop_name, prop_val in props.items():
        if prop_name == "执行日志" and prop_val.get("type") == "rich_text":
            log_text = "".join(
                t.get("plain_text", "")
    ...
    with lock:
        ...
        if len(current_log) >= LOG_ARCHIVE_THRESHOLD:
            ...
            if archived_line:
                ...
                current_log = ""
                props2 = page2.get("properties", {})
                for prop_name, prop_val in props2.items():
                    if prop_name == "执行日志" and prop_val.get("type") == "rich_text":
                        current_log = "".join(
                            t.get("plain_text", "")
                            for t in prop_val.get("rich_text", [])
        ...
        for attempt in range(MAX_LOG_RETRIES):
            try:
                notion_patch(f"pages/{page_id}", {
                    "properties": {
                        "执行日志": {"rich_text": [{"text": {"content": new_log}}]}
                    }
                })
                return True
            except Exception as e:
...
def sync_model_routing_to_notion(page_id: str) -> str:
    """
    将模型路由表同步到 Notion 页面（写入 rich_text block）
    """
    try:
        # 构建 Markdown 格式的路由表
        lines = [
        ...
        for mid, minfo in MODEL_REGISTRY.items():
            lines.append(f"| `{mid}` | {minfo['name']} | {minfo['type']} | {', '.join(minfo['strengths'])} |\n")

        lines.append("\n## 任务路由表\n\n")
        lines.append("| 任务 ID | 描述 | 执行模型 | 审查模型 | 路由原因 |\n")
        lines.append("|--------|------|---------|---------|--------|\n")
        for tid, routing in TASK_MODEL_ROUTING.items():
            if tid.startswith("_"):
                continue
            ...
            lines.append(
                f"| `{tid}` | {routing['description']} | "
                f"{primary_info.get('name', '?')} | "
                f"{reviewer_info.get('name', '?')} | "
                f"{routing.get('reason', '无')[:40]}... |\n"
            )
        ...
        return (f"✅ 模型路由表已同步到 Notion\n"
                f"📋 页面ID: {page_id}\n"
                f"🔗 新建子页面ID: {child_page.get('id', 'unknown')}")
